...
- **Date:** 2026-08-28 · **Branch:** `claude/events-gigs-split-3f9a21` — **Events/Gigs
  split + profile-save fix.** (1) `0025_event_listings.sql`: public `event_listings`
  (+`gigs.event_listing_id`; named that way because bare `events` is 0016's S6 analytics
  stream — found the hard way when `CREATE TABLE IF NOT EXISTS events` silently no-op'd
  against it); seeder now derives 107 listings from the crawled dataset and links all 131
  gigs. (2) Public **`/events`** (upcoming listings, TONIGHT/21+ chips, open-role counts
  → `/gigs?event=<id>` deep link) and public **`/gigs`** (q + role chips, HOT/NEW) —
  every audience can browse both; the landing "Browse Gigs & Events" tab is now a
  CSS hover/focus dropdown (Events / Gigs), and applying stays Talent-only (anon apply
  → 401 → sign-in redirect; matrix row `events.list` PUBLIC). Party sidebar gains
  Discover Events. (3) **Root-caused "can't save Profile/account settings"**: every
  avatar/portfolio picker inlined raw FileReader base64, so photos over ~1.4 MB blew the
  PUT caps (400 schema / 413 body) behind a generic toast — all pickers now go through
  `POST /api/upload` (P4: validated, EXIF-stripped, resized; new `lib/upload-client.ts`
  with a real 5MB gate) and every save mutation surfaces the server's message. (4) The
  Settings "Profile" card moved to My Profile / Venue Profile (shared
  `AccountIdentityCard`; Settings keeps it only for party/admin, who have no profile
  page). Tests 1163 (1142 → +21: listings builder, gig event filter, upload client,
  sidebar) + typecheck/lint/build green; browser-verified live on a disposable Neon
  branch off prod (`dev-events-split`). **Operator remainder: after merge run
  `yarn db:migrate` (0025) + `yarn db:seed-events` against prod** (same pair as the
  S20/0024 apply; prod writes stay permission-gated for the agent).
  population: real NYC events through Oct 2026** so test users have full browse / map /
  venues / search surfaces. Crawled 7 of the 8 requested public sources (Resident
  Advisor, Dice, Eventbrite, Ticketmaster, redeyeny.com, Posh, Ticket Tailor; **Tixr
  skipped** — login-gated, no public city directory) and distilled them into
  `scripts/seed-events.data.ts` (46 real venues w/ coordinates + 131 events → gigs,
  roles varied across DJ/Security/Bartender/Photographer/Go-Go/Bottle Server, 18+/21+
  from the listings, provenance cited per row). New idempotent importer
  `yarn db:seed-events` (venues keyed by email, gigs by venue+title+start; venue users
  are data carriers with discarded random passwords). **Verified end-to-end on a
  disposable Neon branch off prod** (`seed-events-verify`): 0024 applied, import + re-run
  idempotency green, and the app browser-checked live against it (landing Hot Gigs
  Tonight, talent browse w/ HOT/NEW badges, venue directory, FTS search, gig detail w/
  21+ badge + fee estimator + pin). A second branch (`seed-events-sql-rehearsal`)
  rehearsed the same content as pure idempotent SQL. **Prod remains for the operator**
  (session permission gate blocked prod writes): run `yarn db:migrate` (applies pending
  0024 — §7.5 item) then `yarn db:seed-events`, both with the prod owner string.
  discovery & dashboard completion — the last planned development slice.** 1142 vitest
  (975 → 1142) + 21 E2E green against a production build on a disposable Neon branch off
  prod; typecheck/lint/build clean; `db:verify-rls` extended 23 → 28 and run green
  against a real enforcing role; every new surface browser-verified live. Delivery
  summary in §7.3 S20 — headlines: `saved_talent` server persistence (0024, RLS
  in-file) + the conversations `talent_id` anchor with per-card Contact → real thread;
  public `/talent/[id]` profiles (Preview buttons wired, directory/search deep links);
  notifications history page + bell "View all"; gig-detail single-pin map w/
  neighborhood fallback + create-gig live geocode preview (`/api/geocode`, A10-guarded);
  **D3 response rate DEFINED** via a cutover-safe SECURITY DEFINER aggregate; the Q8
  test-gap remainder closed (lifecycles, route-kit, schemas, push/subscribe,
  match-preview). One bug found by live browser verification that the passing component
  tests missed: `?c=` thread selection raced App Router client-side URL updates —
  mechanism replaced (details in §7.3). **All §7 remediation slices S12–S20 are now
  complete; what remains to alpha exit is operator work (§7.5: B2/B4/B5 env keys, the
  RLS `DATABASE_URL` flip per `docs/rls-cutover.md` expecting 28/28, migrations 0024 on
  prod after merge) + the human tabletop + the §11/TESTING.md §11 manual load lab.**
- **Date:** 2026-08-22 · **Branch:** `claude/s19-login-issues-50159f` — **S19 landed: PARTY
  persona completion** (975 vitest + 19 E2E green; typecheck/lint/build clean; details in
  §7.3 S19) **plus a production incident found and fixed the same session**:
  - **Incident (root cause of "can't sign up / can't sign in", reported with a
    `{"status":500}` screenshot):** the production Neon database (`after-dark` /
    `production` branch) had **migrations applied only through 0012** (2026-07-31) while
    deployed `main` shipped 0022 — ten migrations of drift. First casualty: better-auth's
    `rateLimit.storage: 'database'` (S1) writes the `"rateLimit"` table (migration 0013)
    on **every** credential endpoint in production, so all sign-ins/sign-ups 500'd —
    reproduced live (bogus-credential sign-in returned 500, not 401). Also missing: the
    S13 `emailVerified` grandfather (0021), FTS indexes (0015), events/reviews/push/geo
    (0016–0020), telemetry (0022). **Fix:** backup branch (`backup-pre-migrations-0013-0022`,
    expires 2026-09-05) then `yarn db:migrate` with the owner string — 0013–0022 applied
    cleanly; live re-probe now returns 401 with the styled "Invalid email or password"
    error. **Residual for the operator:** the Neon-integration `vercel-dev` branch (created
    2026-08-22, Development env) still sits at 0012 — run `yarn db:migrate` against it or
    reset it from parent; and **the deploy pipeline has no migration step** — running
    `yarn db:migrate --dry-run` (expect "No pending") after every merge, or wiring it into
    CI/CD, is now a standing operator habit (see §4.6 note).
  - S19 delivery summary: become-a-PARTY path (sentinel fix, onboarding card, role-aware
    redirects), `/dashboard` root router (F1 404 fix), party sidebar/bell/messages
    variants, public venue directory (`/api/venues` + `/venues{,/[id]}` + search arm), and
    the server-resolved `venue_id` inquiry path. F8's misrouted search CTA fixed.
...
4. ~~**S14 — money-path integrity**~~ / ~~**S15 — ZAP + gate truthing**~~ — ✅ both
   **landed 2026-08-18** (§7.3). Guardrail-§7 boxes closed across them: webhook signature
   ...
   zero-inline-media gate; the doc-drift list is empty. ~~**S16 — Playwright E2E
   journeys**~~ — ✅ **landed 2026-08-18** (§7.3): the alpha loop per role, DSR/G12
   proofs and the PWA §6.6 real-browser check all run in CI (alpha-gates Gate 1b), and
   the `/account/logout` open-redirect holdout it uncovered is fixed. ~~**S17 —
   frontend truth & component tests**~~ — ✅ **landed 2026-08-18** (§7.3): dead controls
   killed, draft integrity fixed (id-retaining PATCH re-save + publish gates), browse
   search server-side, 24-component-test beachhead. ~~**S18 — RUM & ops
   observability**~~ — ✅ **landed 2026-08-19** (§7.3): first-party RUM beacon,
   per-endpoint Apdex at the real T=300 bar, admin traffic/health cards, Sentry
   source-map wiring. ~~**S19 — PARTY persona completion**~~ — ✅ **landed 2026-08-22**
   (§7.3): become-a-PARTY path, `/dashboard` root router, venue directory + inquiry
   path; same session diagnosed & fixed the production migration-drift auth outage
   (§1 top entry). ~~**S20 — discovery & dashboard completion**~~ — ✅ **landed
   2026-08-26** (§7.3): saved-talent persistence + venue outreach, `/talent/[id]`,
   notifications history, detail/wizard maps, D3 response rate, Q8 test remainder.
   **No development slices remain — the plan through §7 is fully landed.** After the
   merge: run `yarn db:migrate` against prod (adds 0024 — B6 habit), then the
   remaining items are the operator gate (§7.5) and item 5 below.
...
| # | Finding | Evidence |
|---|---|---|
| Q1 | **The frontend is entirely untested**: zero `.test.tsx` files across 28 pages + 53 components; `@testing-library/react` is installed and unused (only the `jest-dom` matchers are wired, for the sw.js sandbox). **→ STARTED in S17** (2026-08-18): 24 component tests across the 5 beachhead targets; the wider sweep continues opportunistically. | `package.json`; repo-wide grep |
| Q2 | **No Playwright E2E exists.** Playwright is installed but its only consumer is the axe smoke. Missing per CLAUDE.md §9: the alpha-loop journey per role, deep-link refresh survival, the **DSR export/delete E2E that is G4's own specified proof**, the signup/age-gate E2E (G12), and the real-browser PWA §6.6 check TESTING.md calls *"a release blocker, full stop"* — currently eyeball-only. **→ CLOSED in S16** (2026-08-18): the `e2e/` Playwright suite runs as CI alpha-gates Gate 1b. | `scripts/axe-smoke.ts:21`; no spec files/config repo-wide |
...
| Q5 | **No RUM**: zero `web-vitals`/`useReportWebVitals` usage. The scorecard rows (CWV p75, per-endpoint Apdex, front-end JS error rate) are unmeasurable in production; guardrail-§7's "RUM dashboards live" box has no implementation at all. **→ CLOSED in S18** (2026-08-19): first-party beacon → `rum_events`, route timings → per-endpoint Apdex at T=300, admin cards. JS error rate stays with Sentry (DSN = operator step B2). | repo-wide grep |
...
| Q8 | **25 of 49 routes have no dedicated test suite.** The consequential ones are the money paths: `payouts/release` (**zero tests** — double-release, cron auth, unkeyed behavior all unasserted), `stripe/webhook` (**zero tests** — guardrail-§7 explicitly requires signature + fee-tamper tests; fee-tamper is argued structurally, never tested), `shifts/[id]` idempotency (k6-only, and that check can no-op), `upload` (caps/MediaError mapping), the DSR routes' two-factor confirm gates, `age-confirm` (G12), `push/subscribe`, `match-preview`; `/api/stream` is tested only with `STREAM_MAX_MS=0` so real-timing window/reconnect behavior is never exercised. Untested utils include **`rls.ts` itself**, `route-kit`, `schemas`, and both lifecycle matrices (`shift-lifecycle`, `application-lifecycle`). **→ worst-of CLOSED across S12–S14:** `rls.ts` (S12); `payouts/release`, `stripe/webhook`, `shifts/[id]` + the explicit fee-tamper, `upload`, DSR confirm gates, `age-confirm` (S14). **Remainder CLOSED in S20** (2026-08-26): `push/subscribe`, `match-preview`, `route-kit`, `schemas`, both lifecycle matrices — 97 dedicated cases (they had been "tracked with S15/S17" but never landed). Stream-under-real-timing stays with the load lab, where real timing exists. | route tree sweep |
...
| F2 | **Dead controls on first-click surfaces**: bare notification-bell buttons with no `onClick` and no unread badge on **8 pages including both dashboard homes** (the real `NotificationsBell` is mounted on only 5 surfaces); create-gig "Preview" has no handler; landing "Talk to Sales" has no handler. **→ CLOSED in S17** (2026-08-18); the two profile-page "Preview" buttons remain, tracked with S20. | `dashboard/talent/page.tsx:256`; `dashboard/venue/page.tsx:282-283`; `create-gig/page.tsx:880-887`; `app/page.tsx:277-284` |
| F3 | **Create-gig draft integrity**: Save Draft never retains the created id, so each click INSERTs a **duplicate draft**; blank fields are silently replaced with fabricated `2026-01-01` dates and "Untitled Gig"; "✓ Draft saved" implies autosave that doesn't exist (the wizard is lost on navigation). **→ CLOSED in S17** (2026-08-18): id-retaining PATCH re-save, real values only, honest label; restoring a draft INTO the wizard on return remains future work. | `create-gig/page.tsx:243-278,862-873` |
| F4 | **Saved Talent is ephemeral**: plain `useState` — not even localStorage — lost on refresh, and filtered against the current results page only; "Message Saved Talent" links to messages with no talent context. **→ CLOSED in S20** (2026-08-26): `saved_talent` server persistence + the conversations `talent_id` anchor — per-card Contact opens the real thread. | `dashboard/venue/browse/page.tsx:249,294-301,594-597` |
| F5 | Notifications have no history surface — bell dropdown only, no View-all page. **→ CLOSED in S20**: paged `/dashboard/notifications` + "View all" from the bell. | `components/NotificationsBell.tsx:64-130` |
| F6 | The gig **detail** page has no map (*"Map view coming soon"* — S10's `GigsMap` is browse-only) and no address show/hide toggle (wireframe p4); create-gig step 2 shows a dead "Map preview" box; the Location card disappears entirely for address-less gigs (neighborhood fallback unused). **→ CLOSED in S20**: single-pin detail map + neighborhood fallback + live wizard preview via `GET /api/geocode`. (The p4 show/hide toggle stays descoped: published-gig addresses are public by design on every card.) | `gigs/[id]/page.tsx:333-344`; `create-gig/page.tsx:464-469` |
| F7 | Browse text-search only filters the currently fetched page on **both** browse surfaces — matching rows beyond page 1 are unfindable by text. **→ CLOSED in S17** (2026-08-18): server-side `q` on both list endpoints, debounced clients. | `dashboard/talent/browse/page.tsx:79`; `dashboard/venue/browse/page.tsx:277-286` |
...
| D3 | Venue "response rate" (p4) | **→ DEFINED in S20** (2026-08-26): share of last-90d inbound conversations the venue answered (non-SYSTEM messages only; hidden below 3 inbound), via the 0024 `app_venue_response_stats` SECURITY DEFINER — cutover-safe. Rendered on the gig-detail venue card and `/venues/[id]`. |
...
| D6 | Admin "uptime / traffic req/s" KPI (p1, PRD §3.4) | **→ CLOSED in S18** (2026-08-19): traffic + API-health cards from `api_timings`; "uptime" is expressed honestly as 5xx rate + cron health (no external prober exists). |
...
- ☑ **38 new route tests where money moves**: `payouts/release` (13 — privilege boundary
  ...
  **explicit §6.4 fee-tamper test**: smuggled fee/gross/net fields never survive into the
  ...
  `age-confirm` (3 — G12; the S16 E2E now carries the end-to-end proof).
...
- ☑ **Gates verified green on real numbers** (not just "CI passed"): `apdex_read_score`
  0.860 @ T=1600 and `apdex_write_score` 0.891 @ T=4000 — nothing frustrated in either
  class, confirming the decomposition the split was built on — p99 4.96 s, ZAP **0
  unaccepted High/Medium** across 9 alerts, verify-rls 23/23, zero-inline-media clean.
...
**S16 — E2E journeys (~3–4 d)** — closes Q2 + the G4/G12 proof gaps —
✅ **landed 2026-08-18** (879 vitest green; 17 Playwright tests green against a
production build on a disposable Neon branch; typecheck/lint/build clean)
- ☑ **Playwright harness on the CI stack** (`playwright.config.ts` + `e2e/`; CI runs it
  as alpha-gates **Gate 1b** against the same running production server the sibling
  gates share, reusing the axe gate's chromium). A `setup` project signs each preview
  role in ONCE and the specs reuse the storage states — deliberate, because CI's
  requests all leave one IP and the S1 limiter allows 10 sign-ins/min (the helper also
  waits out a single 429). Serial (`workers: 1`) over one shared DB; every spec is
  retry-safe via per-attempt unique identities (fresh emails, nonce'd gig titles and
  cover messages) — proven by re-running the suite against the same dirty branch.
- ☑ **The alpha loop as one journey** (`e2e/alpha-loop.spec.ts`): venue publishes through
  the real 4-step wizard → talent finds it in browse, deep-links, checks the estimator's
  −5% math ($200 × 4 h → $760 net), applies with a proposed rate → inquiry thread,
  RATE_PROPOSAL, venue **Accept rate** → shortlist → hire → actor-scoped shift
  transitions split across the two real actors (talent On-My-Way, venue Check-In, talent
  Check-Out, propagating over SSE) → the HELD payout visible on both dashboards. The
  FILLED-gig deep-link regression (hired talent still gets 200 after refresh) and the
  21+ badge (G12's per-gig arm) are asserted inside the journey.
- ☑ **Per-role journeys** (`e2e/admin-party.spec.ts`): a report filed by a real talent is
  triaged OPEN → REVIEWING → CLOSED on the admin dashboard and lands in the audited CSV
  export; non-admins get the access-required card; PARTY browses public surfaces and is
  403-denied on principal writes (gig POST, apply).
- ☑ **G4 DSR proof** (`e2e/dsr-age-gate.spec.ts`): fresh UI signup → export downloads
  with attachment/no-store headers, the documented `meta.excluded`, zero credential
  material — and `age_confirmed_at` stamped, so the G12 attestation proof rides the same
  export → erasure demands BOTH factors (wrong password → 400 and the session stays
  alive) → the dead-cookie canary 401s immediately → deleted credentials cannot sign
  back in. G12's signup arm: Join stays disabled until the 18+ attestation is ticked.
- ☑ **PWA §6.6 in a real browser** (`e2e/pwa.spec.ts`), automating TESTING.md §9's
  manual release blocker: after authed browsing, Cache Storage holds ONLY the static
  shell (precache list + `/_next/static/`) — never an `/api` response or app HTML;
  offline navigation renders the AfterDark offline page and recovers; logout purges
  caches and revokes the session; the manifest is installable (192/512 maskable icons,
  standalone, every icon URL resolves).
- ☑ **Deep links** (`e2e/deep-links.spec.ts`): gig detail and `/search?q=` survive
  refresh; anonymous `/dashboard/*` redirects carry a safe callbackUrl. **Found and
  fixed while writing it: `/account/logout` forwarded a RAW `callbackUrl`** — the one
  unsanitized holdout of §7 finding 4 (signin/signup/two-factor all sanitize).
  `sanitizeCallbackUrl` now guards it and the E2E asserts an absolute URL falls back to
  `/` with the session revoked.
- *Security gate met:* the suite runs entirely on preview-derived credentials (nothing
  logged), the §6.6 negative check fails CI on any authed cache entry, and G4/G12 now
  have executable end-to-end proofs instead of pinned route tests.

**S17 — Frontend truth & component tests (~3 d)** — closes F2, F3, F7 + starts Q1 —
✅ **landed 2026-08-18** (912 vitest green — 879 → 912; 17 Playwright green against a
production build on a disposable Neon branch; typecheck/lint/build clean)
- ☑ **F2 — dead controls killed**: all 8 decorative bells (7 identical dead buttons + the
  talent dashboard's bell-shaped Link) replaced with the real `NotificationsBell` — every
  dashboard header now shows the live unread badge; the bell's role union gained `admin`
  with deep links falling back to `/dashboard/admin` (S19 widens it again for PARTY).
  Create-gig **Preview is wired**: disabled until a draft exists, then opens
  `/gigs/<draftId>` (the owner's real listing view) in a new tab. Landing **Talk to
  Sales** links to `/contact`. Still dead, recorded: the two profile-page "Preview"
  buttons (talent + venue) — they need a public-profile view, tracked with S20's
  discovery work.
- ☑ **F3 — draft integrity**: `PATCH /api/gigs/[id]` gained a strict draft-content arm
  (DRAFT-only, audited as `gig.draft_update`; a body mixing content with `status` is
  rejected so content writes never ride a transition's side effects). The wizard retains
  the created id — first save POSTs, every re-save PATCHes the SAME row, and Publish
  promotes the saved draft (content PATCH + status PATCH) instead of inserting a twin.
  No more fabricated `2026-01-01`/`Untitled Gig` fillers: Save Draft is gated on a real
  title (3+ chars) and empty dates ride as NULL. The saved-state label is honest
  ("Draft saved 10:24 PM — in Open Gigs; re-saving updates it"). Bonus integrity fix:
  the DRAFT→PUBLISHED transition now enforces the same publish gates as the POST path
  (role + start/end + end>start) — previously an incomplete draft could be PATCHed
  straight into the public listing.
- ☑ **F7 — search is server-side**: validated `q` on `GET /api/gigs` and `GET /api/talent`
  (same LIKE-substring haystack the client filter scanned, now across the whole listing);
  both browse pages send it debounced (350 ms) with a page reset, and the client-side
  text filter is gone — matches beyond page 1 are findable. Builder suites prove `q`
  stays parameterized (SQLi cases extended to cover it).
- ☑ **Q1 beachhead — 24 component tests** with the previously-unused
  `@testing-library/react` (jsdom stubs for ResizeObserver/matchMedia added to setup):
  `NotificationsBell` (badge/9+ cap, role-aware links incl. the admin fallback,
  mark-all-read, signed-out degradation), `DashboardSidebar` (all three role navs with
  pathname-driven group auto-expand, real session name, unread-badge sum),
  `MessagesView` (thread list, recipient-only Accept-rate — **and the behavioral
  render-XSS case S15 promised this harness**: hostile content renders as inert text),
  the browse page (cards, filter panel → validated server params, the debounced `q`
  regression), and the create-gig wizard (all five F3 contracts).
- ☑ **CI-stack truthing the first runs forced** (both latent S16 Gate-1b interactions,
  surfaced by this branch's CI): the Lighthouse gate's sign-in now rides out one
  rate-limit window (the runner IP shares the 10 sign-ins/min budget with the axe + E2E
  gates — same policy as e2e/fixtures.ts), and the k6 setup keys its apply→hire chain on
  a **burstable** (SCHEDULED/IN_TRANSIT) shift instead of list emptiness — the E2E loop
  legitimately leaves talent.preview with a CHECKED_OUT shift, which made the old check
  skip the chain and then fail the S15 no-vacuous-idempotency guard. (The S16 PR's own
  CI run had already tripped this at the k6 step; it was merged before that surfaced.)
- *Security gate met:* draft PATCH is owner-scoped under the existing matrix row with a
  404 for rivals; publish gates close the incomplete-listing hole; the behavioral XSS
  test complements the S15 structural gate.

**S18 — RUM & ops observability (~2 d)** — closes Q5, D6 — ✅ **landed 2026-08-19**
(934 vitest green — 912 → 934; 18 Playwright green incl. a new observability journey;
typecheck/lint/build clean)
- ☑ **First-party RUM** (Q5, G11-clean): `WebVitalsReporter` (`useReportWebVitals`,
  production-only, mounted in providers) beacons LCP/CLS/INP/FCP/TTFB to `POST /api/rum`
  via sendBeacon — same origin, no third-party collector. The ingest boundary is layered:
  strict zod schema (unknown keys rejected — an identity field cannot ride along),
  server-side path re-normalization (UUIDs/digit-runs → `[id]`, on BOTH sides), a final
  character allowlist, 2 KB body cap, and a per-IP limiter (240/min — one NAT'd IP fronts
  many visitors). Anonymous by design; the matrix invariant "no anonymous marketplace
  writes" gained an explicit, justified single-entry exception list.
- ☑ **Per-endpoint Apdex from route-kit** (Q5): `withRoute` now captures every request's
  `(route-name, method, status, duration)` — the route NAME, never a URL — into
  `api_timings`, **buffered and flushed as one UNNEST batch per ~25 rows / 5 s** so the
  capture's cost is amortized to ~zero (`TELEMETRY_SAMPLE` knob remains; skipped under
  vitest; an instance that dies loses its ≤25-row unflushed tail — acceptable for
  telemetry, on the record). The buffering is not premature: **the first CI run measured
  a naive per-request insert dragging the k6 read-Apdex from 0.86 to 0.79** on the
  co-hosted runner — telemetry that degrades the thing it measures is self-defeating,
  and loosening the calibrated gate to absorb it would have been gate-erosion. The
  admin overview derives per-endpoint and request-weighted Apdex at **T=300 ms — the
  real §3 scorecard bar**, not CI's calibrated tripwire.
- ☑ **Telemetry storage done properly** (migration `0022_telemetry.sql`): `rum_events` +
  `api_timings` are RLS-governed append-only tables — INSERT/DELETE for the SERVICE
  context only, SELECT for ADMIN/SERVICE, UPDATE revoked outright; grants in the living
  set. The daily retention purge prunes both at 30 days (docs/retention.md row added) —
  telemetry can never become an unbounded PII-adjacent log.
- ☑ **Admin traffic/uptime cards** (D6, wireframe p1): API Requests (24h) with req/min,
  API Apdex (24h, T=300) with 5xx count + slowest endpoint, LCP p75 and INP/CLS p75
  (7d RUM) — all real aggregates on `/api/admin/overview`, honest "no data yet" empty
  states. Cron health (S14) stays on the escrow card.
- ☑ **`withSentryConfig` source-map wiring**: next.config wraps only when
  `SENTRY_AUTH_TOKEN` is set (org/project/token in `.env.example`) — unkeyed builds stay
  byte-identical; the runtime SDK remains separately DSN-keyed.
- ☑ **Proof at every layer**: 22 new vitest cases (rum lib normalization/rating, capture
  writers incl. never-throw + SERVICE context + clamping, ingest-route strict-validation
  matrix incl. smuggled-identity rejection and 429s) + an E2E observability journey on
  the real stack: anonymous beacon → 204, smuggled field → 400, the suite's own traffic
  visible in the admin overview, and the four new cards rendering.
- *Security gate met:* beacon payloads carry no PII and no ids at all (stricter than the
  gate asked — role was dropped too); ingest rate-limited; telemetry tables are
  SERVICE-write-only, admin-read, 30-day bounded.

#### Functionality wave

**S19 — PARTY persona completion** — ✅ **landed 2026-08-22**, closes F1, F8 (975 vitest
+ 19 E2E green on the real stack; typecheck/lint/build clean)
- ☑ **Become-a-PARTY path**: the dead `'PERSONAL'` signup sentinel now writes `PARTY`
  and personal signups route through onboarding (pre-S19 they never got a role at all);
  onboarding gained a Party People card whose flow is a single step (no profile —
  §6.3: role only); completion redirects are role-aware via one shared
  `dashboardPathFor()` (`src/lib/use-my-role.ts`), PARTY landing on `/venues`.
- ☑ **`/dashboard` root router** (the F1 universal 404): reads the session role and
  forwards — ADMIN/VENUE/TALENT homes, PARTY → venue discovery, roleless → onboarding.
  The post-2FA redirect and the landing nav now target `/dashboard` instead of
  hardcoding the talent home.
- ☑ **Party UI variants, exhaustively**: `DashboardSidebar` gained a `party` nav
  (Discover Venues / Messages / Search) and — the actual F1 bug — role lookups are now
  exhaustive `Record<DashboardRole, …>` maps, so an unknown role can never silently
  render the venue nav again; `NotificationsBell` widened with party deep links
  (message → party inbox, anything else → `/venues`, mirroring the admin fallback);
  `MessagesView` accepts `party` with its own empty state; settings accepts
  `?role=party`.
- ☑ **Venue discovery**: public `GET /api/venues` (+ `/api/venues/[id]`) on a
  public-column projection (**no user_id, no street address in the list**; detail serves
  address exactly as every gig card's venue join already does) via a new pure builder
  (`venue-query.ts`, mirrored on talent-query); `/venues` directory page (debounced
  search, infinite pages) + `/venues/[id]` detail; a `venues` arm in `/api/search` +
  GlobalSearch/`/search` sections; migration `0023_venue_directory.sql` adds the 0015-
  doctrine FTS GIN index + a partial directory-ordering index.
- ☑ **Inquiry path**: `POST /api/conversations` accepts `venue_id` (public
  venue_profiles.id) and resolves the venue's user **server-side** — user ids still
  never ride the client; "Inquire about a private party" CTA on the venue page
  (hidden for VENUE viewers; anon → sign-in with callback); rate proposals stay
  hidden on PARTY_INQUIRY threads; seed adds `party.demo@afterdark.test` + a
  venue↔party inquiry so both sides have local data. F8: the search page's Message
  CTA is now rendered for VENUE viewers only (it sent every role to the venue inbox).
- ☑ **Proof**: 41 new/extended vitest cases (venue builder + directory/detail routes
  incl. SQLi parameterization and 404-on-non-uuid, search venue arm, conversations
  venue-anchor semantics — PARTY_INQUIRY kind, gig-anchor 403 for PARTY, venue↔venue
  400, unknown-venue 404 — sidebar/bell/messages party variants) + a new E2E party
  journey on the production stack: `/dashboard` → `/venues` redirect, directory +
  detail render, venue-anchored inquiry creates a PARTY_INQUIRY, party inbox renders
  it. The S12 RLS-wiring gate caught both new files on first run — declared
  `public-read`/`builder` with the other anon surfaces.
- *Security gate met:* the standing "PARTY is never a principal" matrix invariant stays
  green (E2E re-proves 403s live); the venue directory leaks nothing beyond public
  profile columns (unit-asserted projection); inquiries are rate-limited
  (`conversations-create`) and reportable (P5 report flow, asserted in the messages
  component suite).

**S20 — Discovery & dashboard completion** — ✅ **landed 2026-08-26**, closes F4, F5, F6,
the D3 decision, the S17 Preview-button leftover, and the Q8 test-gap remainder (1142
vitest + 21 E2E green; typecheck/lint/build clean; every surface browser-verified live
against a disposable Neon branch off prod)
- ☑ **F4 — saved talent is server truth**: migration `0024_discovery_dashboard.sql` —
  `saved_talent` keyed (venue **user** id, public `talent_profiles.id`), composite PK,
  RLS in-file (owner-only in all directions — no public read, no platform carve-out),
  GRANTs SELECT/INSERT/DELETE with UPDATE revoked (no update shape exists). API:
  `GET /api/venue/saved-talent` (joined public card columns, so the rail survives
  paging) + `PUT` (idempotent save/unsave, `ON CONFLICT DO NOTHING`, rate-limited,
  audit-exempt on the record). Browse page: optimistic heart toggle, rail rows from the
  server list with per-row unsave/message/profile links; the dead "Message Saved
  Talent" group CTA is gone. Export coverage: `saved_talent` added to
  `account-data.ts` (stage-name joined for portability); erasure rides the user FK
  cascade; retention row added.
- ☑ **F4b — venue outreach without user ids**: `POST /api/conversations` gained a
  `talent_id` anchor (mirrors S19's `venue_id` — the server resolves the talent's user
  id; VENUE-only, PARTY/TALENT 403). Card "Contact" and the rail's message button open
  (or dedupe into) the real thread and land on `/dashboard/venue/messages?c=<id>`;
  `MessagesView` honors the `?c=` deep link. **Two bugs found by live verification
  that the (passing) component tests could not see**: (1) reading `window.location`
  in a mount effect races App Router client-side pushes — the URL can still be the
  previous page's when effects first run, so the wrong thread got selected; fixed by
  consuming the param when the conversations list arrives (post-round-trip, URL
  settled), once — after that the user's own clicks win. (2) The first saved-talent
  implementation was optimistic (instant heart flip + rollback) — the E2E exposed
  that a late-landing stale list response silently clobbers the optimistic entry,
  and a click inside that inconsistent window INVERTS the intent (a save becomes an
  unsave); redesigned non-optimistic — heart state is server truth, hearts disable
  while a toggle is in flight. Inquire flows (gig detail, venue detail) now carry
  `?c=` too.
- ☑ **F5 — notifications history**: `GET /api/notifications` gained `?page=` (sentinel
  +1 pagination; page 1 byte-compatible with the bell's read); `/dashboard/
  notifications` renders the paged feed for all four roles (role via `?role=` +
  session fallback), Load-older, Mark-all-read; "View all notifications" footer in the
  bell dropdown; a Notifications entry in all four sidebar navs. `describeNotification`
  exported — one text/deep-link mapping for bell + page.
- ☑ **F6 — maps**: gig detail renders a single-pin `GigsMap` (new `className`/
  `showSummary`/`interactive`/`maxZoom` props; browse behavior unchanged) inside the
  Location card, which now always renders — address, else neighborhood + "exact
  address after you're hired"; "Map view coming soon" is gone. Create-gig step 2:
  the dead placeholder is a live preview — debounced (800 ms) `GET /api/geocode`
  (VENUE-only, 15/min/user, reuses the S10 `geocodeAddress` through the A10 safe-fetch
  allowlist; sub-5-char input answers `{point:null}` with zero network; nothing
  persisted — publish still geocodes authoritatively).
- ☑ **D3 — response rate DEFINED** (not dropped): share of last-90-day inbound
  conversations (first non-SYSTEM message from the counterpart) the venue answered
  with a non-SYSTEM message afterwards; NULL below 3 inbound so a one-sample 100%
  can't read as reputation. Computed by `app_venue_response_stats` — SECURITY DEFINER
  (0017 doctrine: conversations/messages stay participant-private, the public card
  needs only the aggregate; a bare aggregate would silently zero post-cutover).
  Rendered on the gig-detail venue card and `/venues/[id]` (which also now shows its
  fetched-but-never-rendered `rating_count`). Verified on-branch: 3 inbound / 1
  replied → 33%; SYSTEM banners never count.
- ☑ **S17 leftover — Preview buttons wired**: both profile editors open the real
  public view in a new tab (`/talent/[id]` / `/venues/[id]`), disabled until the
  first save creates the row. The talent side needed the page to exist: public
  `GET /api/talent/[id]` + `/talent/[id]` (mirrors the S19 venue detail — public
  columns + p9's portfolio/socials, socials rendered as inert text never hrefs, no
  shift aggregates because those are participant-private under RLS). Directory
  cards, the saved rail, and search talent results now deep-link to it (§5.3).
- ☑ **Q8 remainder closed** (promised with S15/S17, never landed): dedicated suites
  for `shift-lifecycle` + `application-lifecycle` (full actor×edge sweeps),
  `route-kit` (ApiError mapping, opaque 500s, telemetry capture), `schemas` (31
  contract cases incl. the S20 schemas), `push/subscribe` (key-gate, re-homing,
  scoping), `match-preview` (shape, definer-only availability, 400s).
- ☑ **Proof**: 1142 vitest (975 → 1142: ~15 new matrix rows' generated tests, 89
  S20 route/migration cases, 15 component cases across 5 suites incl. the new
  browse + history pages, 97 Q8-remainder cases); `db:verify-rls` 23 → **28 checks**
  (saved_talent owner-only ×3 + forge-rejection; response-stats context-free) — run
  green against a real enforcing role on the disposable branch; E2E grew 19 → 21
  (`/talent/[id]` anonymous deep link; venue save→refresh→Contact→thread→unsave
  loop) and the full suite passed against a production build on the branch; the
  0024 structural block joined `marketplace-migrations.test.ts`.
- *Security gate met:* saved_talent leaks nothing (no public read; the GET projection
  is unit-asserted user-id-free); talent user ids never ride the client anywhere in
  the outreach path; the geocode probe stays inside the A10 allowlist and is
  role-gated + rate-limited; response-rate exposes counts-only aggregates through a
  pinned-search-path definer; all three machine-checked registries (authZ matrix,
  RLS wiring, audit coverage) gained their S20 rows on day one.
...
landed — once its PR merges, the flip is re-opened; follow `docs/rls-cutover.md`. Update
2026-08-26: S20 grew the suite — expect **28/28**.)*

**B6 (new, 2026-08-22 — from the migration-drift incident, §1):** production migrations
are a manual operator step and drifted 10 files behind the deployed code, taking every
credential endpoint down with 500s. Now that 0001–0023 are applied: after **every** merge
that adds a `migrations/*.sql`, run `DATABASE_URL=<owner> yarn db:migrate` (or verify
`--dry-run` prints "No pending") **before or immediately after the Vercel deploy** — or
wire a migration step into CI/CD. The Neon-integration `vercel-dev` branch (Development
env) still sits at 0012 and needs the same command once, if it is kept.
